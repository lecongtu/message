...
//  MessengerApp
//
//  Created by Lê Công Tú on 08/09/2021.
//
...
class ProfileViewController: UIViewController {
    ...
    func createTableHeader() -> UIView? {
        ...
        let nameView = UILabel(frame: CGRect(x: (headerView.width-150) / 2,
                                             y: 185,
                                             width: 150,
                                             height: 150))
        
        database.child("\(safeEmail)").child("first_name").getData(completion: {error , snapshot in
        ...
        database.child("\(safeEmail)").child("last_name").getData(completion: {error , snapshot in
            guard error == nil else {
                print(error!.localizedDescription)
                return
            }
            fullname = "\(fullname) \(snapshot.value as? String ?? "")"
